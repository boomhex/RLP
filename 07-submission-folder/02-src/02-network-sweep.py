...
def parse_args() -> argparse.Namespace:
    parse = argparse.ArgumentParser(description="Train dynamics model")
    parse.add_argument("--datadir", type=Path, default=Path("../01-data"),
                   help="Directory containing preprocessed data x_train.pt, y_train.pt, ...")
    parse.add_argument("--resultsdir", type=Path, default=Path("../03-results"),
                   help="Directory where results must be stored")
    parse.set_defaults(shuffle=True)
...
if __name__ == "__main__":
    ...
    noise_frac = 0.5  # 20% of y rows become wrong (permuted)
    ...
    out_path = Path(f"./depth{depth}_noise{int(noise_frac)}_wmin{width_min}_wmax{width_max}_epochs{epochs}.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)

    with out_path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["depth", "width", "epoch", "loss_train", "loss_test", "net_loss"])
        for width in tqdm(range(width_min, width_max, step)):

            net = Network(in_dim=x_train.size(1), out_dim=y_test.size(1), h_layers=depth, h_size=width)
            print(f"width:{width}, depth:{depth}")
            for ep in tqdm(range(0, epochs, 1)):
