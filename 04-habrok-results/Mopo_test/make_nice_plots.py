...
# CSV_PATHS = [
#     "good_results_big1.csv",
#     "good_results_big2.csv",
#     "good_results_big3.csv",
# ]
...
# what to plot on Y:
# use_test_only = True   -> error = loss_test
# use_test_only = False  -> error = loss_test - loss_train
use_test_only = True

WIDTH_PLOT_NO_SMOOTH_FIRST_N = 1
...
PLOT_RAW_TOO = False    # overlay raw curve faintly
...
def smooth_with_first_n_raw(series: pd.Series, win: int, first_n_raw: int) -> pd.Series:
    s = series.rolling(window=win, center=SMOOTH_CENTER, min_periods=1).mean()
    n0 = int(first_n_raw)
    if n0 > 0:
        s.iloc[:n0] = series.iloc[:n0]
    return s
...
def main():
    ...
    width_slice["error_smooth"] = smooth_with_first_n_raw(
        width_slice["error"], SMOOTH_WIN_WIDTH, WIDTH_PLOT_NO_SMOOTH_FIRST_N
    )
    width_slice["train_smooth"] = smooth_with_first_n_raw(
        width_slice["train"], SMOOTH_WIN_WIDTH, WIDTH_PLOT_NO_SMOOTH_FIRST_N
    )

    plt.figure()
    if PLOT_RAW_TOO:
        plt.plot(width_slice[WIDTH_COL], width_slice["error"], alpha=0.25, linewidth=1)
        plt.plot(width_slice[WIDTH_COL], width_slice["train"], alpha=0.25, linewidth=1)
    ...
    plt.figure()
    if PLOT_RAW_TOO:
        plt.plot(epoch_slice[EPOCH_COL], epoch_slice["error"], alpha=0.25, linewidth=1)
        plt.plot(epoch_slice[EPOCH_COL], epoch_slice["train"], alpha=0.25, linewidth=1)
